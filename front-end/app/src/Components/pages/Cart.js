...
export default function Cart() {
  ...
  return (
    <div>
      ...
      <div className="cart">
        {(cart).map((cart) => (
        <div className="item"  style={{ border: "2px solid black", marginTop:10 }}>
          <center>
            <h2 style={{marginTop: 5}}>{cart.ItemName}</h2>
            <h3>Inventory Quantity: {cart.Availability}</h3>
            <h3 style={{color: "red", marginBottom:10}}> Cost: {cart.Price}</h3>
          </center>


          <div className ="quantity">   
            <div style={{marginLeft:50, float:"right"}}>
                <button className="button_2" style={{alignItems: "right"}} onClick={remove(cart)}>
                  {" "}
                  Remove{" "}
                </button>
              </div>
           ...
           <button className="button_1" style={{ float: "right" }} onClick={increase_qty(cart)}>
              {" "}
              +{" "}
            </button>
