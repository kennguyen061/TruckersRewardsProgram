...
const retrievecart = (UID, SID) => {
    db.query("SELECT * FROM CARTITEM WHERE UID = ? AND SID = ?",
    [
        UID,
        SID
    ],
...
const getquantity = (UID, SID,ItemID) => {
    db.query("SELECT Quantity FROM CARTITEM WHERE UID = ? AND SID = ? AND ItemID = ?",
    [
        UID,
        SID,
        ItemID
...
function addtocart(UID, SID, ItemName, Price, Quantity) {
    db.query("INSERT INTO CARTITEM(UID,SID,ItemName,Price,Quantity) VALUES(?,?,?,?,?)",
    [
        UID,
        SID,
        ItemName,
...
function removeitemfromcart(UID, SID, ItemName) {
    db.query("DELETE FROM CARTITEM WHERE UID = ? AND SID = ? AND ItemName = ?",
    [
        UID,
        SID,
        ItemName
...
function addquantity(UID, SID, ItemID) {
    ...
    db.query("UPDATE CARTITEM SET Quantity = ? WHERE UID = ? AND SID = ? AND ItemID = ?",
    [
        newQuantity,
        UID,
        SID,
...
function updateprice(UID, SID, ItemID, Price) {
    db.query("UPDATE CARTITEM SET Price = ? WHERE UID = ? AND SID = ? AND ItemID = ?",
    [
        Price,
        UID,
        SID,
...
function changename(UID, SID, ItemID, ItemName) {
    db.query("UPDATE CARTITEM SET ItemName = ? WHERE UID = ? AND SID = ? AND ItemID = ?",
    [
        ItemName,
        UID,
        SID,
